...
### 目前未解决的问题
1. 前端删除admin功能优化与单个单元格可触发更新
2. 前端修改密码去除自动填充密码，并且优化样式
3. 后端amdin新增操作控制枚举类，对mybatisplus进行枚举映射
4. 对系统级操作权限进行，针对不同操作每个用户有不同权限，只有管理员才能修改其他用户的权限，管理员还可以指定用户为员工staff，让其拥有修改非管理员和员工的权限
5. 个人密码更新接口的设计
6. 对缓存工具类进行优化，选择使用自定义数据结构或使用linkedHashMap数据结构
7. 新增redis缓存功能，当用户依赖redis并设置redis时，可以通过配置改变缓存的存储方式
